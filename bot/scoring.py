...
def compute_overdue_multiplier(due_at: Optional[datetime], reference: datetime) -> float:
...
def compute_recurrence_multiplier(recurrence: Optional[str]) -> float:
    if not recurrence:
        return 0.0
...
def compute_domain_balance_multiplier(domain_id: Optional[int], recent_domain_totals: Dict[int, float]) -> float:
...
def within_daily_window(last_completed_at: Optional[datetime], completed_at: datetime) -> bool:
    if last_completed_at is None:
        return False
    return abs((completed_at - last_completed_at).total_seconds()) <= 30 * 60 * 60


def within_weekly_window(last_completed_at: Optional[datetime], completed_at: datetime) -> bool:
    if last_completed_at is None:
        return False
