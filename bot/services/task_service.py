...
def complete_task(
    session: Session, task: Task, reference: datetime, recent_domain_totals: Dict[int, float]
) -> Completion:
...
def add_subtask(
    session: Session, task: Task, title: str, order_idx: Optional[int] = None
) -> SubTask:
...
def neglected_domains(
    session: Session, user_id: int, recent_domain_totals: Dict[int, float]
) -> List[int]:
...
def compute_task_projection(
    task: Task, reference: datetime, recent_domain_totals: Dict[int, float]
) -> XPBreakdown:
    streak_value = task.streak.current_streak if task.streak else 0
